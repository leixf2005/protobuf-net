...
namespace ProtoBuf
{
    readonly struct DoubleBufferedReadOnlyBuffer
    {
        ...
        {
            private ReadOnlyBuffer.Enumerator _wrapped;
            private bool _isFirst;
            private readonly ReadOnlyMemory<byte> _first;
            public Enumerator(ReadOnlyMemory<byte> first, ReadOnlyBuffer.Enumerator enumerator) : this()
            {
                _first = first;
                _wrapped = enumerator;
                _isFirst = true;
        ...
        internal DoubleBufferedReadOnlyBuffer Slice(Position start)
        {
            var relativeStart = Start;
            if (relativeStart.Segment != start.Segment) return new DoubleBufferedReadOnlyBuffer(_buffer.Slice(start));

            return Slice(start.Index - relativeStart.Index);
        }
        internal DoubleBufferedReadOnlyBuffer Slice(int start, int length)
            => new DoubleBufferedReadOnlyBuffer(_buffer.Slice(start + _consumed, length));

        public DoubleBufferedReadOnlyBuffer Slice(int start)
        {
